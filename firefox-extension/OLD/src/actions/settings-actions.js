...
export function bindSettingsAdders(app) {
  ...
  addEduBtn.addEventListener("click", () => addEdu(app));   // creates first/next entry & shows inputs
  addExpBtn.addEventListener("click", () => addExp(app));
  addLangBtn.addEventListener("click", () => addLang(app));
  addSkillBtn.addEventListener("click", () => addSkill(app)); // shows inline add input for a chip
}

// Keep, but now it only renders what exists; no auto-adds.
