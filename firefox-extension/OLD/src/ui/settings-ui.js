...
import { save, load, saveSettingsJSON } from "../storage/storage.js";

/** PUBLIC API */
export function renderAllLists(app) {
  ensureSaveButton(app);  // Save validates -> if ok, persists; else shows error
  renderEdu(app);
  renderExp(app);
  renderLang(app);
  renderSkill(app);
...
function ensureSaveButton(app) {
  ...
  btn.title = "Save settings to local storage";

  btn.addEventListener("click", async () => {
    // Make sure app.data.settings has the latest basics before validating
    ...
    if (!v.ok) {
      // Show error and highlight basic fields if applicable
      setStatus(app, v.msg, true);
      applyBasicErrorStyles(app, v.missingKeys || []);
      return;
    }

    // Build exact JSON and persist
    const exactJson = buildExactSettingsJSON(app.data.settings);

    try {
      await save(app.data);            // persist full app state
      await saveSettingsJSON(exactJson); // persist exact schema JSON
      const fresh = await load();        // reload to hydrate
      if (fresh) {
        app.data = fresh;
        fillBasicsFromState(app);
        renderAllLists(app);
      }
      setStatus(app, "Settings saved.", false);
    } catch {
 ...
 * Required:
 *  - summary
 *  - email, linkedin, name, role, phone (mobile), address
 *  - languages: >= 1
 *  - experiences: >= 1
 *  - skills (coreSkills): >= 1
...
function validateSettings(s) {
  ...
  let msg = "";
  if (missing.length) {
    msg = "Please fill all required fields: " + missing.join(", ");
  ...
  // At least one experience should have role or company
...
function buildExactSettingsJSON(s = {}) {
  ...
  // Education: single object in target schema -> first item or empty
...
// Pull current inputs into app.data.settings before saving
